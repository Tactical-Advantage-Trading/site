...
title = "1. Claim your ECX balance"
  ...
  title = "Install eWallet"
  image = "images/1-1.png"
  image_alt = "eWallet welcome screen with options to create or restore a wallet"
  content = """Install eWallet from Play Store, or download the APK directly from the [GitHub releases page](https://github.com/Tactical-Advantage-Trading/wallet/releases).
...
**Do not install an app from any other place except those two links above! Doing so will most likey result in loss of funds**."""
  ...
  image_alt = "eWallet dialog for entering a 12-word recovery phrase"
...
**Never enter the phrase shown in this screenshot. It is just a disposable example**."""
  ...
  image_alt = "eWallet showing three ECX wallet types and one BTC wallet"
  content = """Once confirmed, eWallet derives three ECX wallet types: BIP84 (labeled as eCash), BIP44, and BIP32, along with BTC BIP84 wallet (labeled as Bitcoin). 
...
To do it, start by tapping the first **ECX eCash** wallet card shown in the previous section. This opens your ECX BIP84 receiving addresses. From there on, proceed with this tutorial."""
  ...
  image_alt = "eWallet showing four ECX BIP84 receiving addresses"
  content = """Choose any of the four available ECX addresses by tapping it, or by sharing it back to eWallet.

This address belongs to your eWallet. It is the destination that will receive your ECX while replay protection is activated, so the funds will always remain under your control."""

  [[claim_sections.steps]]
  title = "Select every funded ECX wallet"
  image = "images/2-2.png"
  image_alt = "eWallet asking which funded ECX wallets to spend from"
  content = """Select every ECX wallet that contains funds. eWallet will combine their balances into a single ECX transaction.
...
In this example, all three ECX wallet types contain funds, so all three are selected."""
  ...
  image_alt = "eWallet send dialog with the maximum ECX amount selected"
  ...
  image_alt = "eWallet confirmation dialog showing the ECX amount and chain fee"
  ...
  image_alt = "eWallet showing ECX consolidated in the default wallet with a pending transaction"
...
Hiding them is optional. It simply keeps the wallet view tidy and does not move any funds."""
  ...
  image_alt = "eWallet settings view with Hide buttons below the wallet cards"
  ...
  image_alt = "eWallet after the empty legacy ECX wallet cards have been hidden"
...
Only hide cards with a zero balance. Keep the default **ECX eCash** card visible."""
  ...
  image_alt = "eWallet showing only the default ECX and BTC wallet cards"
